...
def operation(v):
    ...
    #print(v_)
    v = np.concatenate((v_, np.array(v[8:])))
    ...
    plotus.append(ans/8)
    dobavka = 0
    for i in range(dln, dln + 12):
        dobavka += min(abs(v[i] - np.pi/2), abs(v[i])) / 12
...
# guess вар1
guess = np.zeros((1,dln+12))[0]

# Добавляем рандомные сдвиги к элементам guess
delta = np.pi
for i in range(dln):
    guess[i] = guess[i] + random.randint(-1000, 1000) / 1000 * delta
for i in range(dln, dln+6):
    guess[i] = random.randint(0, 1000) / 1000 * np.pi/2
for i in range(dln+6, dln+12):
    guess[i] = random.randint(0, 1000) / 1000 * np.pi/2
